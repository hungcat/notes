...
開発モード（`docs:dev`）で起動中、ナビゲーションメニューの「メモ入力」からアクセスできます（`/compose`）。
...
├── docs/
│   ├── .vitepress/
│   │   ├── components/     # MemoEditor.vue 等の UI コンポーネント
│   │   ├── util/           # API 実装、Git 操作、スラグ変換等のユーティリティ
│   │   ├── config.ts       # VitePress 設定（API プラグインの登録）
│   │   └── sidebar.data.ts # サイドバーの動的生成ロジック
│   ├── memo/               # 生成されたメモ（.md）の保存先
│   └── compose.md          # エディタ画面のエントリポイント
├── package.json
└── tsconfig.json
```
